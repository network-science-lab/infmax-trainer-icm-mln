...
from _data_set.nsl_data_utils.loaders.constants import (
    AND, EXPOSED, OR, PEAK_INFECTED, PEAK_ITERATION, SIMULATION_LENGTH
)
...
class MLNInfo:
    ...
    @staticmethod
    def _validate_args(icm_protocol: str, icm_p: float, x_type: str, y_type: str) -> None:
        ...
        assert set(y_type).issubset({EXPOSED, SIMULATION_LENGTH, PEAK_ITERATION, PEAK_INFECTED})

    @staticmethod
    def _get_ft_path(x_type: str, mln_type: str, mln_name: str) -> Path | None:
        if x_type == "zeros":
                return None
