...
class NeighborhoodGen:
                ...
                if len(cur_nbrs) > 0:
                    if len(walk) == 1 or prev_layer_id != cur_layer_id:
                        walk.append(
                            cur_nbrs[
                                alias_draw(
                                    alias_nodes[cur_layer_id][cur][0],
                                    alias_nodes[cur_layer_id][cur][1],
                                )
                            ]
                        )
                    else:
                        ...
                        next = cur_nbrs[
                            alias_draw(
                                alias_edges[cur_layer_id][(prev, cur)][0],
    ...
    def preprocess_thread(self, layer, counter):
        ...
        for node in layer.nodes():
            unnormalized_probs = [
                layer[node][nbr]["weight"] for nbr in sorted(layer.neighbors(node))
            ]
            norm_const = sum(unnormalized_probs)
            normalized_probs = [
