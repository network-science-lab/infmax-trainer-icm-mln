...
class LightningHeteroData(HeteroData):
    ...
    def from_network_info(
        cls, network_info: MultilayerNetworkInfo, output_dim: int, input_dim: int
    ) -> Self:
        ...
        for idx, _ in enumerate(network_info.net_pt.layers_order):
            layer_edge_indexes = network_info.net_pt.adjacency_tensor[idx, ...].coalesce().indices()
        ...
        data[ACTOR].z = network_info.net_pt.nodes_mask.T
        ...
        data.actors_map = bidict(
            {
                ...
                actor, actors_map in network_info.net_pt.actors_map.items()
            }
        )
        data.layers_map = bidict(
            {
                l_name: f"l_{l_idx}" for
                l_idx, l_name in enumerate(network_info.net_pt.layers_order)
            }
        )
    ...
    def _prepare_features(network_info: MultilayerNetworkInfo, input_dim: int) -> torch.Tensor:
        ...
        if network_info.x_type == "zeros":
            ...
            mln_centralities: list[dict[MLNetworkActor, float]] = [
                centrality_function(network_info.net_nd)
                for centrality_function in CENTRALITY_FUNCTIONS[:input_dim]
            ]
            actor_indices, features_raw = [], []
            for actor in network_info.net_nd.get_actors():
                actor_indices.append(network_info.net_pt.actors_map[actor.actor_id])
                features_raw.append(
                    [
