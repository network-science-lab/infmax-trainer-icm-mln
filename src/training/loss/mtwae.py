...
class MTWMSE(torch.nn.Module):
    def __init__(
        ...
        """
        Mean Total Weighted Mean Squared Error.
    ...
    def forward(self, y_hat: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        wmse = se * self.weights.expand_as(se)  # weighted mse
        twmse = wmse.sum(dim=1)  # total wmse
        mtwmse = twmse.mean()  # compute mean total weighted mse in the batch
