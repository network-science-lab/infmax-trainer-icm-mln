...
def train(args):
    ...
    for net in networks:
        ...
        pred_performance = evaluate_seed_set(net.graph, pred_seeds, proto, p, n_steps, n_repetitions)
        print(f"Predicted seed set: {pred_seeds}")
        print(f"{pred_performance.mean()}\n")

        ref_seeds = get_gt_data(net.name, proto, p, seed_size)
        ref_performance = evaluate_seed_set(net.graph, ref_seeds, proto, p, n_steps, n_repetitions)
